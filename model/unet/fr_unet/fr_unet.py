...
# forked from: https://github.com/lseventeen/FR-UNet
...
class FR_UNet(nn.Module):
    def __init__(self,  in_channels=3, out_channels=1, feature_scale=2,  dropout=0.2, fuse=True, out_ave=True):
        ...
        self.final1 = nn.Conv2d(
            filters[0], out_channels, kernel_size=1, padding=0, bias=True)
        ...
        self.final5 = nn.Conv2d(
            filters[0], out_channels, kernel_size=1, padding=0, bias=True)
        self.fuse = nn.Conv2d(
            5, out_channels, kernel_size=1, padding=0, bias=True)
